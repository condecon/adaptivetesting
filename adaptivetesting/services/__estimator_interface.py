...
class IEstimator(ABC):
    def __init__(self,
        ...
        """
        if type(response_pattern) is not jnp.ndarray:
            self.response_pattern = jnp.array(response_pattern)
        else:
            self.response_pattern = response_pattern
        self.optimization_interval = optimization_interval

        # convert items to parameter arrays
        self.a = jnp.array([i.a for i in items])
        self.b = jnp.array([i.b for i in items])
        self.c = jnp.array([i.c for i in items])
        self.d = jnp.array([i.d for i in items])
